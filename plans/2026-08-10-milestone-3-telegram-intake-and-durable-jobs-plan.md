...
- 2026-08-13 — M3 platform wiring + E2E selesai. Seed migration `20260813100000_seed_allowlist_admin.sql` (admin `83540732`/`alamaby`) applied ke dev via `migrate-development.yml` (9/9, hosted tests 79/79; workflow run 31670269521 awalnya gagal karena `EXPECTED_MIGRATIONS` belum include seed → fixed `540c430`). Vercel Preview env vars diset (APP_ENV=development, TELEGRAM_BOT_TOKEN, TELEGRAM_WEBHOOK_SECRET, JOB_PROCESSOR_SECRET, SUPABASE_URL dev, SUPABASE_SERVICE_ROLE_KEY dev, PROVIDER_KEY_ENCRYPTION_KEY dev — key baru dibuat karena terlewat M2). Deployment Protection dimatikan untuk Preview (menyebabkan 401 "Protected deployment" pada webhook). Webhook diset: `https://albot-6vpa38l30-alam-aby-bashits-projects.vercel.app/api/telegram/webhook` (allowed_updates message+callback_query). E2E: prompt "desain poster kafe cozy di malam hari" → balasan "Prompt diterima. Sedang dalam antrian...", DB row: prompt_sessions `received`, prompt_revisions `pending`, jobs `enhance_prompt` `queued`, telegram_updates unik berurutan (668535295-298), active-session guard menolak prompt kedua. Bot token yang terekspos di chat di-revoke & diganti.
