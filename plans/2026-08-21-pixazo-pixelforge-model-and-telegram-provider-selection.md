...
- 2026-08-22 — Dev migrate `87e23ee` gagal types drift → regen `database.types` (FK `prompt_sessions_preferred_image_provider_config_id_fkey`, `isOneToOne:true`, order) via `npm run db:types` → commit `87e23ee` types, `schema.integration` fix FK name; re-run migrate dev 23/23 aman (forward-fix `20260822100000/22110000` + types now match). User konfirm migrate development aman.
