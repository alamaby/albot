...
- 2026-08-26 — migrate-dev run 1 (`c92b4e8`) gagal di types check → F8 regen union `b76cc42`; run 2 (`fc207aa`) gagal di hosted (bug select test saya + flaky dedupe) → fix `15fd84a`; **run 3 `2323678` SUCCESS** (32952466785, hosted 126/126); **migrate-production SUCCESS** (32953970159, prod 25→26, attestation match). T-9 migration DONE. Sisa: deploy verifikasi + setMyCommands dev/prod + E2E Telegram + docs.
