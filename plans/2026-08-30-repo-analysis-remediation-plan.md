...
- 2026-08-31 11:30 — Run deploy-development setelah user set dashboard Node 22.x + token: `vercel build` CLI 47 sekarang complain `Found invalid Node.js Version: "24.x"` di project setting (meskipun dashboard sudah 22.x — CLI cache project setting). Repo guardrail `engines.node: "22.x"` masih belum cukup.
- 2026-08-31 14:50 — **PIVOT user**: workflow deploy-development terlalu rumit. User disable workflow + pilih deploy manual via Vercel CLI. Plan baru: `plans/2026-08-31-nexjs16-vercel-adapter-and-manual-deploy-pivot.md`.
- 2026-08-31 15:35 — **CLOSED via pivot**: `@next-community/adapter-vercel@0.0.1-beta.29` + peer build-utils di `dependencies` (`bb4c885`); `adapterPath` di `next.config.ts` (`f150b01`); `deploy-development.yml` → `.yml.disabled` (`43b32cb`); runbook `docs/runbooks/manual-deploy.md` (`a5dcbb0`). Thread CI deploy-development ditutup lewat path manual, bukan bukti hijau. Plan Node 22 (`2026-08-31-fix-deploy-development-node-22.md`) dan plan ini (Phase 4 final user action) **CLOSED**.
