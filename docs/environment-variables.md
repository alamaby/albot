...
- Development dan production harus memiliki nilai berbeda untuk semua variable di bawah ini (bot dev dihapus — hanya prod yang punya `TELEGRAM_*`/`JOB_*`; dev retained hanya untuk Supabase staging).
...
- Migration: `migrate-development` (staging, auto push) dan `migrate-production` (prod, auto push — Opsi C, hapus gate `development_run_id`; lihat `plans/2026-09-02-bot-dev-removal-and-auto-prod-plan.md`). CLI Supabase memakai `SUPABASE_ACCESS_TOKEN` dan `SUPABASE_DB_PASSWORD` environment-specific.
...
| `TELEGRAM_BOT_TOKEN`          | Production only                                | `required` (bot prod `@albot_ai_bot`; dev bot dihapus)   | Milestone 3          |
| `TELEGRAM_WEBHOOK_SECRET`     | Production only                                | `required` (min 8 char URL-safe base64)                  | Milestone 3          |
| `JOB_PROCESSOR_SECRET`        | Production only (+ `recovery-production` cron) | `required` (min 32 char)                                 | Milestone 3          |
...
### `development` (staging migration-only — bot dev dihapus)
...
> Bot development dihapus (`plans/2026-09-02-bot-dev-removal-and-auto-prod-plan.md`): `TELEGRAM_BOT_TOKEN` / `TELEGRAM_WEBHOOK_SECRET` / `JOB_PROCESSOR_SECRET` dev tidak lagi disimpan. Hapus dari Settings → Environments → `development` + delete environment `recovery-development`.
...
### Milestone 1 (diperbarui Opsi C — `plans/2026-09-02-bot-dev-removal-and-auto-prod-plan.md`)

- GitHub Environments `development` (staging) dan `production` (prod-only bot).
- Secret migration/CLI per environment seperti di atas (prod standalone).
- `migrate-development` auto on `push main` ke dev staging; `migrate-production` auto on `push main` ke prod (Opsi C — hapus gate `development_run_id`/attestation, verifikasi hanya `EXPECTED_REF=pcexxtckvwmiquseznaz`); `deploy-production` auto `workflow_run` setelah prod success (T7a).
