...
- Supabase projects: dev `ceqcitzbosqzxpbtlpfn` (**49 migrations** — 43 repo lama + 3 Aichixia/reasoning-pref 03 Sep + **nemotron-rotation `20260904090000` + bot-text seeds `20260904120000` + stuck-received `20260904150000` (applied dev via MCP `recover_stuck_received_sessions_dev`)**), prod `pcexxtckvwmiquseznaz` (**50 migrations — `20260904090215` orphan history + `20260904150000` re-apply aligned 04 Sep via `fix(db): align production migration history 04090215` 7dd0f9f**; migration list match repo)
- Production topology: Vercel `albot-be.alamaby.com` (manual `vercel --prod` deploy 04 Sep; auto-deploy `deploy-production.yml` menunggu `VERCEL_TOKEN` diisi env `production`), webhook `@albot_ai_bot`, allowlist `83540732`, provider reasoning: Cloudflare gpt-oss-120b (0) → Pollinations gpt-oss (150) → OpenRouter free ×5 (200–230, round_robin) → Bynara laguna 161 + 5 nemotron-replacement 162–166 round_robin; image: Pixazo 0/5 → **Aichixia flux2/lucid/phoenix/gemini (110–113; dev & prod ter-keyed 03 Sep — fingerprint 43658ea80bdf...)** → Pollinations flux (151) → Bynara 160-180 → picker 200-203. Picker Telegram: 8 model image + 13 model reasoning (per-sesi + default per-user). Kunci provider terenkripsi AES-256-GCM via `upsert-provider-key.mjs`.
- **2026-09-04 16:20 — Stuck received guard**: sesi prod `45209362` `received queued 0` stuck 68m (P95) tanpa error — webhook ok `prompt_received` tapi `POST /api/jobs/process` claim tidak jalan (H1 idle vs H2 after-lease hilang; H3 recovery 20m terlalu lambat). `claim_job` manual di prod membuktikan claimable. Fix: RPC `recover_stuck_received_sessions(p_max 25, p_stuck 2m)` + `RecoveryRepository` + `runRecovery` 1c (after stuck-generating) + Telegram `enhancement_failed` + `retryKeyboard`. `after()` dipertahankan per user (plan 2026-09-04-diagnose...). `EXPECTED_MIGRATIONS` + `EXPECTED_FUNCTIONS` update (49 prod, 48 dev + duplicate `20260904150000`).
...
- **Uncommitted 2026-09-04 (siap 1 commit)**: migration `20260904090000` (5 reasoning Bynara 162–166 + deactivate nemotron-3-ultra) + registrasi adapter + picker 8→13 + `FailureContext` di semua pesan gagal (provider + code + HTTP + safeMessage redacted) **+ migration `20260904120000` (5 bot/LLM keys) + `BotTextRepository` + DEFAULT_ maps + async get* wrappers + reasoning options wiring + ~30 call sites**. Unit 357/357, contract 107/107, `db:types:check` OK (no DDL). Build lokal OK kecuali symlink EPERM Windows (pre-existing).
