...
Last updated: 2026-09-04 12:00 (DB-driven prompt keys 1→6 + bot text/keyboard DB-configurable; negative prompt per-model (no global default); seed migration 20260904120000 applied to dev, pending prod migrate — detail di `2026-09-04/120000-universal-prompt-configurability.md`)
...
- Supabase projects: dev `ceqcitzbosqzxpbtlpfn` (**48 migrations** — 43 repo lama + 3 Aichixia/reasoning-pref 03 Sep + **nemotron-rotation `20260904090000` + bot-text seeds `20260904120000` (seeded dev via MCP 04 Sep; `migration list` dev = file)**), prod `pcexxtckvwmiquseznaz` (**47 migrations — `20260904090000` routed via prod MCP but prod file list at 03 Sep was 46; `20260904120000` pending `migrate-production`**)
...
- **2026-09-04 12:00 — Prompt controllability follow-up**: `enhancement_system_persona` + `reasoning_revision_helper` + `reasoning_sampling` + `bot_messages`/`bot_keyboards`/`bot_templates` = **6 keys aktif** (DB-driven). LLM helper/sampling strict-error; bot text/keyboard fallback-ke-default (agar bot tidak bisu). `DEFAULT_BOT_MESSAGES` 45 keys + `DEFAULT_BOT_TEMPLATES` 28 keys + `DEFAULT_KEYBOARD_LABELS` 15 keys. Negative prompt: tanpa default global (kosong = omit; Pixazo/Aichixia-difusi native, gemini omit, Pollinations/Bynara `Avoid:`).
